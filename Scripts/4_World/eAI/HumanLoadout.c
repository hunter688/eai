class HumanLoadout {	
	ref TStringArray Shirts = {"TShirt_Grey"};
	...
	ref TIntArray	 LootChance = {100, 100, 100, 100, 100, 100, 100, 100};					//Add item from Loot array
...
//		HumanLoadout.AddWeapon(h, weapon);
		...
		EntityAI gun = h.GetHumanInventory().CreateInInventory(sidearm);
		float HealthModifier = (Math.RandomInt(Loadout.WeaponHealth[0], Loadout.WeaponHealth[1])) / 100;
		gun.SetHealth(gun.GetMaxHealth() * HealthModifier);
		HumanLoadout.AddMagazine(h, sidearm, Loadout.WeaponRifleMagCount[0], Loadout.WeaponRifleMagCount[1]);
		
		for (int i = 0; i < Loadout.Loot.Count(); i++) {
			if (Loadout.LootChance.Count() <= i)
				Loadout.LootChance.Insert(100);
			if (Math.RandomInt(0, 100) < Loadout.LootChance[i])
				h.GetInventory().CreateInInventory(Loadout.Loot[i]);
		}
