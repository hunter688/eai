class eAIDynamicPatrol {
	...
	// Override these two functions for easier custom behavior (change faction, behavior, etc...)
	void OnPatrolSpawn(eAIGroup patrol) {}
	void OnUnitSpawn(eAIBase unit) {}
	...
	eAIGroup SpawnPatrol() {
		...
		Print("ACTIVE GROUP " + m_activeGroup);
		...
		while (count > 1) {
			eAIBase unit = game.SpawnAI_Helper(ai, m_loadout);
			unit.RequestTransition("Rejoin");
			OnUnitSpawn(unit);
			count--;
		}
		OnPatrolSpawn(m_activeGroup);
